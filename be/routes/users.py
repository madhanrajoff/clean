...
from models import Users, UserUpdate

router = APIRouter()


@router.post("/", response_description="Create a new user", status_code=status.HTTP_201_CREATED, response_model=Users)
def create_user(request: Request, user: Users = Body(...)):
...
@router.get("/", response_description="List all users", response_model=List[Users])
def list_users(request: Request):
    users = list(request.app.database["users"].find(limit=100))
    return users


@router.get("/{id}", response_description="Get a user by id", response_model=Users)
...
@router.put("/{id}", response_description="Update a user", response_model=Users)
